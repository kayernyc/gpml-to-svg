...
import { ValidateFileNameResult, validateFileName } from "./fileNameValidation";
...
describe("validateFileName", () => {
	...
	test("should return TooShort and InvisibleFile for a file name that is too short and begins with a period", () => {
		const fileName = ".a";
		const result = validateFileName(fileName);
		expect(result.message).toBe(`The proposed filename ".a" is too short. Please make the filename at least 3 characters long.
